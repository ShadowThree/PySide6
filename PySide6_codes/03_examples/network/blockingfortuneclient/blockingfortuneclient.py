"""PySide6 port of the network/blockingfortuneclient example from Qt V5.x, originating from PyQt"""
...
class BlockingClient(QWidget):
    def __init__(self, parent=None):
        ...
        host_label = QLabel("&Server name:")    # '&' for shortkey
        ...
        for ip_address in QNetworkInterface.allAddresses():
            if ip_address != QHostAddress.LocalHost and ip_address.toIPv4Address() != 0:
                break
        else:   # only executed if not break
            ip_address = QHostAddress(QHostAddress.LocalHost)   # 127.0.0.1
        ...
        host_label.setBuddy(self._host_line_edit)   # bind the label shortkey to the lineedit 
        port_label.setBuddy(self._port_line_edit)

        self._status_label = QLabel("This example requires that you run the Fortune Server example as well.")
    ...
    def request_new_fortune(self):
        self._get_fortune_button.setEnabled(False)
        self.thread.request_new_fortune(self._host_line_edit.text(), int(self._port_line_edit.text()))

    def show_fortune(self, nextFortune):
        """if the fortune is same with last one, request new one"""
