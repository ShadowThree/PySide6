from PySide6.QtWidgets import (QWidget, QApplication, QMessageBox, QLineEdit, QProgressBar,
                               QPushButton, QHBoxLayout, QVBoxLayout, QStyle, QFileDialog)
...
class DownloaderWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._open_folder_action = self.dest_box.addAction(
            qApp.style().standardIcon(QStyle.SP_DirOpenIcon),   # Icon
            QLineEdit.TrailingPosition                          # position
            # QLineEdit.LeadingPosition
        )
        self._open_folder_action.triggered.connect(self.on_open_folder)  # set the icon slot
        ...
        self.dest_box.setText(      # C:/Users/userName/Downloads
    ...
    def on_start(self):
        ...
        dest_path = QDir.fromNativeSeparators(self.dest_box.text().strip())     # strip(): remove the space default
        dest_file = QDir(dest_path).filePath(url_file.fileName())
        print(dest_path)    # C:/Users/userName/Downloads
        print(dest_file)    # C:/Users/userName/Downloads/qt-everywhere-src-6.0.1.zip

        if QFile.exists(dest_file):
            ret = QMessageBox.question(
                self, "File exists", "Do you want to override the file?",
